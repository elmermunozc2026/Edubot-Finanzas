...
if mod_idx == 0:
    ...
    cols = st.columns(4)
    quick_queries = [
        (t("cfo_quick_ratios", lang), f"Calcula ratios: AC=2.5M, PC=1.2M, ventas=8M, UN=1.2M, patrimonio=5M, EBITDA=2M, deuda=3M. Empresa sector {sector}." if lang=="es" else f"Calculate ratios: CA=2.5M, CL=1.2M, sales=8M, NI=1.2M, equity=5M, EBITDA=2M, debt=3M. Sector {sector}."),
        (t("cfo_quick_caja", lang), "Posición de caja: BCP=850K, BBVA=420K, Scotiabank=180K, cobros=300K, pagos=450K, líneas=2M." if lang=="es" else "Cash position: BCP=850K, BBVA=420K, Scotiabank=180K, collections=300K, payments=450K, lines=2M."),
...
# ── 2: PLANEAMIENTO ───────────────────────────────────────────────────────────
    ...
    with tab_c:
        with st.form("form_capex"):
            proj=st.text_input(t("cfo_nombre_proyecto",lang),value="Expansión Planta" if lang=="es" else "Plant Expansion")
            c1,c2=st.columns(2)
            with c1:
                inv_ini=st.number_input("Inversión/Investment (S/):",value=5_000_000.0,step=100_000.0)
                wacc=st.slider("WACC:",0.05,0.25,0.12,0.01,format="%.0f%%")
                vida=st.number_input("Vida útil/Life (años/years):",value=5,min_value=1,max_value=20)
            with c2:
            ...
            if st.form_submit_button(t("cfo_evaluar_capex",lang),type="primary",use_container_width=True):
                q=(f"Evalúa CAPEX '{proj}': inversión={inv_ini}, flujos={flujos}, WACC={wacc:.2f}, vida={vida} años."
                   if lang=="es" else
    ...
    with tab_ca:
        with st.form("form_caja"):
            ...
            with c3:
                ...
                lineas=st.number_input("Líneas/Lines (S/):",value=2_000_000.0,step=100_000.0)
            if st.form_submit_button(t("cfo_calcular_posicion",lang),type="primary",use_container_width=True):
                ...
                q=(f"Posición de caja: saldos={json.dumps(saldos)}, cobros={cobros}, pagos={pagos}, líneas={lineas}."
                   if lang=="es" else
    ...
    with tab_ba:
        with st.form("form_banco"):
            ...
            with c1:
                ...
                tipo_opts={"es":["Línea revolvente","Préstamo a plazo","Carta fianza","Factoring","Leasing"],
            ...
            if st.form_submit_button(t("cfo_analizar_linea",lang),type="primary",use_container_width=True):
                q=(f"Línea bancaria {banco}: tipo={tipo}, monto={monto}, usado={usado}, tasa={tasa:.4f}, vencimiento={venc}."
                   if lang=="es" else
...
# ── 4: RIESGOS ────────────────────────────────────────────────────────────────
    ...
    with tab_ri:
        with st.form("form_riesgo"):
            nombre_r=st.text_input(t("cfo_nombre_riesgo",lang),value="Caída precio cobre" if lang=="es" else "Copper price drop")
            c1,c2=st.columns(2)
            with c1:
            ...
            if st.form_submit_button(t("cfo_evaluar_riesgo_btn",lang),type="primary",use_container_width=True):
                q=(f"Evalúa riesgo '{nombre_r}': cat={cat}, prob={prob}, impacto={imp}, ingresos={ing}."
                   if lang=="es" else
    ...
    with tab_vr:
        with st.form("form_var"):
            ...
            if st.form_submit_button(t("cfo_calcular_var",lang),type="primary",use_container_width=True):
                q=(f"VaR: portafolio={port}, volatilidad={vol}, confianza={conf}, horizonte={hor} días."
                   if lang=="es" else
    ...
    with tab_ir:
        with st.form("form_ir"):
            ...
            with c2:
                ...
                reg=st.selectbox("Régimen/Regime:",list(TAX_REGIMES[lang].keys()),format_func=lambda x:TAX_REGIMES[lang][x])
            if st.form_submit_button(t("cfo_calcular_ir",lang),type="primary",use_container_width=True):
                q=(f"IR: utilidad={util}, adiciones={adic}, deducciones={dedu}, pagos_cuenta={pagos_c}, régimen={reg}."
                   if lang=="es" else
    ...
    with tab_igv:
        with st.form("form_igv"):
            ...
            if st.form_submit_button(t("cfo_calcular_igv",lang),type="primary",use_container_width=True):
                q=(f"IGV: débito={igv_v}, crédito={igv_c}, saldo_anterior={saldo_ant}."
                   if lang=="es" else
...
# ── 6: CONTROL INTERNO ────────────────────────────────────────────────────────
    ...
    with c2:
        ...
        if st.button(t("cfo_ver_obligaciones",lang),use_container_width=True):
            q=(f"Obligaciones regulatorias mes {mes_reg}, sector {sector}, Perú."
               if lang=="es" else
...
# ── 7: REPORTING ──────────────────────────────────────────────────────────────
    ...
    with st.form("form_reporting"):
        ...
        with c2:
            ...
            logro1=st.text_input("1:",value="Reducción costos 5%" if lang=="es" else "5% cost reduction")
            logro2=st.text_input("2:",value="Renovación línea BCP S/ 3M" if lang=="es" else "BCP line renewal S/ 3M")
            logro3=st.text_input("3:",value="Cero observaciones auditoría" if lang=="es" else "Zero audit findings")
